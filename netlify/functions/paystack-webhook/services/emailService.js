...
class EmailService {
  ...
  async sendOrderConfirmation(orderRef, orderData, paymentData) {
    ...
    try {
      ...
      if (emailLog.exists && emailLog.data().confirmationSent) {
  ...
  async sendPaymentFailureNotification(email, details) {
    ...
    try {
      ...
      const emailLog = await emailLogRef.get();
      if (emailLog.exists && emailLog.data().failureNotificationSent) {
        console.log(`Failure notification already sent for order ${details.orderId}`);
        return;
      }
